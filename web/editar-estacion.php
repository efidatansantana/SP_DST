...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id_eess == '' || $terminal == '' || $productos == '' || $nombre_eess == '' || $id_empresa == '') {
        Header("Location: editar-estacion.php?error=1&id_eess=" . $id_eess . "&id_empresa=" . $id_empresa);
    } else {
        if ($funciones->updatePrefenrenciaEstablecimiento($id_eess, $nombre_eess, $productos, $terminal, $estado, $id_empresa)) {
    ...
    if (isset($_GET['id_eess']) && isset($_GET['id_empresa'])) {
        // Guardamos los datos del formulario
        $id_eess = $_GET['id_eess'];
        $id_empresa = $_GET['id_empresa'];

        $estacion = $funciones->getEstacion($id_eess, $id_empresa);
        if ($estacion == null) {
...
<?php if (isset($_GET['id_eess'])): ?>
    ...
    <div class="container">
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" id="form-estacion">
            ...
            <!-- CORREGIDO: id_empresa como hidden para que llegue por POST -->
            ...
            <div class="row pt-2">
                <div class="col">
                    <label for="terminal" class="form-label">(*)Código Terminal</label>
            ...
            <div class="row pt-2">
                <div class="col">
                    ...
                    <select class="selector-est" name="productos[]" multiple id="selected_producto" required>
                        ...
                        $productos_establecimiento = $funciones->getProductosEstablecimiento($id_eess, $id_empresa);
                    ...
                    <small class="text-muted">Selecciona los productos que desea que se envíen los precios.</small>
                </div>
                <div class="col">
                    <label for="selected_empresa" class="form-label">(*)Empresa</label>

                    <select class="selector-est" name="empresa" required>
                        <?php
                        $empresas = $funciones->getEmpresas();
                        $empresas_asignadas = array_column($funciones->getEstablecimientoEmpresa($id_eess), 'CODIGO_EMPRESA');
                        foreach ($empresas as $empresa) {
                            $selected = in_array($empresa['CODIGO'], $empresas_asignadas) ? 'selected' : '';
                            echo '<option value="' . $empresa['CODIGO'] . '" ' . $selected . '>' . $empresa['EMPRESA'] . '</option>';
                        }
                        ?>
                    </select>
                    <small class="text-muted">Selecciona la empresa a la que pertenece la estación. Las empresas se añaden desde <a href="empresas.php" target="_blank">empresas</a>.</small>
                </div>
            </div>
            ...
            <a href="listado-estaciones.php?empresa=&estacion=<?php echo $estacion['ID_EESS_MITYC']; ?>" class="btn btn-warning mt-3">
                <i class="bi bi-arrow-left-circle-fill"></i> Volver atrás
            </a>
            <button type="submit" class="btn btn-success mt-3" id="sav">
