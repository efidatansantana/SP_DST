...
if (isset($_GET['action'])) {
    if ($_GET['action'] == 'delete') {
        if (isset($_POST['id_eess']) && isset($_POST['id_empresa'])) {
            // Guardamos los datos del formulario
            $id_eess = $_POST['id_eess'];
            $id_empresa = $_POST['id_empresa'];
            if ($id_eess  == '' && $id_empresa  == '') {
                Header("Location: listado-estaciones.php?error=" . $_GET['id_eess']);
            } else {
                if ($funciones->deleteEstablecimiento($id_eess, $id_empresa)) {
    ...
    <table class="table table-strMunicipioed sortable">
        ...
        <tbody>
            ...
            foreach ($estaciones as $estacion) {
                ...
                echo "<td>" . $estacion['NOMBRE_EMPRESA'] . "</td>";
                ...
                echo "
                        ...
                        <td class='text-center'>
                            <a href='#' class='btn btn-danger btn-sm' data-bs-toggle='modal' data-bs-target='#modalEliminar' data-bs-ide='" . $estacion['ID_EESS_MITYC'] . "' data-bs-idempresa='" . $estacion['CODIGO_EMPRESA'] . "'>
                                <i class='bi bi-trash'></i>
                            </a>

                            <a href='editar-estacion.php?id_eess=" . $estacion['ID_EESS_MITYC'] . "&id_empresa=" . $estacion['CODIGO_EMPRESA'] . "' class='btn btn-primary btn-sm'>
    ...
    <script>
        $(document).ready(function() {
            $('#modalEliminar').on('show.bs.modal', function(event) {
                ...
                var id_empresa = button.data('bs-idempresa') // Extract info from data-bs-* attributes
